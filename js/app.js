(function ($) {
    $(function () {
        ...
        function setSpeedFromMillis(val = 1000) {
            ...
            speed = parseInt(val);
            slideVal = parseInt(min + (max - min) / (700 - 3000) * (speed - 3000));
        ...
        function setSpeedFromSlider(val = 1) {
            ...
            var min = parseInt(speed_slider.prop('min'));
            var max = parseInt(speed_slider.prop('max'));
            speed = parseInt(3000 + (700 - 3000) / (max - min) * (val - min));
            $('.btn-steps').attr('data-arg2', speed).data('arg2', speed);
        ...
        function sendCommand(command, successFunc = function () { }) {
            ...
            $.ajax({
                ...
                error: function () {
                    ...
                    M.toast({
                        html: '<i class="material-icons">close</i>&nbsp;&nbsp;&nbsp;No response from device',
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    var modalInstall = $('#modal-install').modal({
        dismissible: false,
    });
    modalInstall.modal('open');
